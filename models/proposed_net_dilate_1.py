...
class TCN(nn.Module):
  def __init__(
    ...
    for i in range(layer_num): 
      if self.dilated: 
        self.TCN.append(
            DepthConv1d(BN_dim, hidden_dim, kernel_size, dilation=1, padding=1, skip=skip, causal=causal))
      else:
...
class Dual_Tas_Model_dilated(nn.Module):
  def __init__(self, config):
    super(Dual_Tas_Model_dilated,self).__init__()
    self.config = config
...
if __name__ == "__main__":
  rnn = Dual_Tas_Model_dilated(256, 64, 128, bidirectional=True, norm='ln', num_layers=6)
  from torchinfo import summary
