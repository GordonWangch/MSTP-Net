...
class Dual_Tas_Block(nn.Module):
  def __init__(self, configs):
    super(Dual_Tas_Block, self).__init__()
  ...
  def forward(self, x):
    ...
    intra = x.permute(0, 2, 1, 3).contiguous().view(B*K, F, S)      ## [BK, F, S] -> [bs * 100, 64, 22]
    intra = self.TCN_local(intra)                                   ## [BK, F, S] -> [bs * 100, 64, 22]
    intra = intra.view(B, K, F, S)                                  ## [B, K, F, S] -> [bs, 100, 64, 22]
    intra = intra.permute(0, 2, 1, 3).contiguous()                  ## [B, F, K, S] -> [bs, 64, 100, 22]
    ...
    # inter for global
    # [B, F, K, S] -> [B, K, F, S] -> [B * K, F, S]
    inter = intra.permute(0, 2, 1, 3).contiguous().view(B*K, F, S)  ## [BK, F, S] -> [bs * 100, 64, 22]
    ...
    out = inter + intra                                             ## [B, F, K, S] -> [bs, 64, 100, 22]
...
class Dual_Path_Tas(nn.Module):
  def __init__(self, configs):
    ...
    # Dual-Path-Tas
    self.dual_tas = nn.ModuleList([])
    for _ in range(self.stack_num):
      self.dual_tas.append(Dual_Tas_Block(self.configs))
  ...
  def forward(self, x):
    ...
    for i in range(self.stack_num):
      x = self.dual_tas[i](x)                 ## [B, F, K, S] -> [bs, 64, 100, 22]
...
class Dual_Tas_Model_all_global(nn.Module):
  def __init__(self, config):
    super(Dual_Tas_Model_all_global,self).__init__()
    self.config = config
...
if __name__ == "__main__":
  rnn = Dual_Tas_Model_all_global(256, 64, 128, bidirectional=True, norm='ln', num_layers=6)
  from torchinfo import summary
