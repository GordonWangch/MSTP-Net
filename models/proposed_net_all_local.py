...
class Dual_Tas_Block(nn.Module):
  ...
  def forward(self, x):
    ...
    inter = intra.permute(0, 3, 1, 2).contiguous().view(B*S, F, K)  ## [BS, F, K] -> [bs * 22, 64, 100]
    inter = self.TCN_global(inter)                                  ## [BS, F, K] -> [bs * 22, 64, 100]
    inter = inter.view(B, S, F, K)                                  ## [B, S, F, K] -> [bs, 22, 64, 100]
    inter = inter.permute(0, 2, 3, 1).contiguous()                  ## [B, F, K, S] -> [bs, 64, 100, 22]
...
class Dual_Tas_Model_all_local(nn.Module):
  def __init__(self, config):
    super(Dual_Tas_Model_all_local, self).__init__()
    self.config = config
...
if __name__ == "__main__":
  rnn = Dual_Tas_Model_all_local(256, 64, 128, bidirectional=True, norm='ln', num_layers=6)
  from torchinfo import summary
